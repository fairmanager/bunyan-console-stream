...
class ConsoleStream extends Stream {
  constructor ( config = {} ) {
    ...
    const errThreshold = [ 10, 20, 30, 40, 50, 60 ].includes( parseInt( config.stderrThreshold ) ) ? config.stderrThreshold : 70;

    this.streamConfig = {
      stderrThreshold : errThreshold,
      displayStack    : Boolean( config.displayStack )
    }
  }
  ...
  write( data ) {
    ...
    if( data.level >= this.streamConfig.stderrThreshold ) {
...
module.exports.createStream = config => new ConsoleStream( config );
